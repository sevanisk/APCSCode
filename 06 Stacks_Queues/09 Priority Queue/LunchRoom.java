...
public class LunchRoom
{
   ...
   public static void main(String[] args)
   {
      ...
      int se = 0, ju = 0, so = 0, fr = 0;
      int seWait = 0, juWait = 0, soWait = 0, frWait = 0;
      int longestSeWait = 0, longestJuWait = 0, longestSoWait = 0, longestFrWait = 0, currWait = 0;
      double seTWait = 0, juTWait = 0, soTWait = 0, frTWait = 0;
      ...
      //adds new customers
         ...
         if(z == 1 && x < 1080) 
         {
            int clas = (int)(Math.random() * 4);
            queue.add(new Customer(x, clas));
            if(clas == 0)
               fr++;
            else if(clas == 1)
               so++;
         ...
         if(!queue.isEmpty())
         {
            if(!queue.isEmpty() && queue.peek().getWin() && queue.peek().getD() == x)
            {
               currWait = queue.peek().getD() - queue.peek().getA();
               if(queue.peek().getClas().equals("Se"))
               {
                  seWait = currWait;
                  seTWait += seWait;
                  if(seWait > longestSeWait)
                     longestSeWait = seWait;
               }
               else if(queue.peek().getClas().equals("Ju"))
               {
                  juWait = currWait;
                  juTWait += juWait;
                  if(juWait > longestJuWait)
                     longestJuWait = juWait;
               }
               else if(queue.peek().getClas().equals("So"))
               {
                  soWait = currWait;
                  soTWait += soWait;
                  if(soWait > longestSoWait)
                     longestSoWait = soWait;
               }
               else if(queue.peek().getClas().equals("Fr"))
               {
                  frWait = currWait;
                  frTWait += frWait;
                  if(frWait > longestFrWait)
                     longestFrWait = frWait;
               }
               queue.remove();
            }
           
            
            if((!queue.isEmpty() && !queue.peek().getWin()))
            {
               queue.peek().timeAdjust(x);
               queue.peek().setWindow(true);
            }
         
         
         }     
         //displays Queue
         if(x < 1088 || !queue.isEmpty())
         {
            PriorityQueue<Customer> newQ = queue;
            display(x, newQ);
            System.out.println("");
         }
      ...
      System.out.println("Senior      " + se + "       " + longestSeWait + "        " + (double)seTWait/se);
      System.out.println("Junior      " + ju + "       " + longestJuWait + "        " + (double)juTWait/ju);
      System.out.println("Sophomore   " + so + "       " + longestSoWait + "        " + (double)soTWait/so);
      System.out.println("Freshmen    " + fr + "       " + longestFrWait + "        " + (double)frTWait/fr);  
   }
...
class Customer implements Comparable<Customer>
{
   ...
   public Customer(int arrivalTime, int clas)
   {
   ...
   public void setWindow(boolean f)
   {
      atWindow = f;
   }
  ...
  //returns the new departure time after adjusting it and the wait time
   ...
   {
      if(obj.getClas().equals(year))
         return 0;
      else if(obj.getClas().equals("Fr"))
         return -1;
      else if (obj.getClas().equals("So") && year.equals("Fr"))
         return -1;
      else if (obj.getClas().equals("Jr") && (year.equals("Fr") || year.equals("So")))
         return -1;
      else if (obj.getClas().equals("Se"))
         return -1;
      return 1;
   }
}
