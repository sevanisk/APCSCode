...
//| List Total: 16 | Tree Total: 13 |  Savings: 19%
   class SpellNode
   {
      ...
      // Searches for ch in the children of this node.
      //     if pos==chldrn.size() then ch was NOT found  
      }  
      int pos_of_char(char ch) 
      {
         for(int x = 0; x < chldrn.size(); x++)
         {
            if(chldrn.get(x).val == ch)
               return x;
         }
         return chldrn.size();
   ...
   public class SpellCheckTree
   {
      public static void main(String[] args) throws FileNotFoundException
      {
         ...
         Scanner myW = new Scanner(new File("widgets.txt"));
         String word = myW.next();
         int letters = (word).length(); 
         //int pos = 0;
         for(int y = 0; y < letters; y++)
         {
            int pos = pos_of_char(word.charAt(y));
            if(pos == chldrn.size())
            {
               chldrn.add(new SpellNode(word.charAt(y)));
               list_total++;
            }
         }
      }
